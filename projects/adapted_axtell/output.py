...
def save_general_agents_data(my_agents, decision):
    # Checking whether the file of the same name exists. If TRUE, delete the file

    if os.path.isfile(os.path.join(parameters.OUTPUT_PATH, "temp_general_agents_pop_%s_decision_%s_time_%s.txt" %
            (parameters.pop_reducer, decision, parameters.final_Time))) is True and statistics.my_stats.time == 0:
...
def save_general_firms_data(my_stats, max_size, total_effort, mean_agent_effort, decision):
    # Check whether the file has the same name. If TRUE, delete the file
    ...
    firms_txt.write('%s, %s, %.2f, %.2f, %.2f, %.2f, %.2f , %.2f, %.2f, %.2f \n' % (my_stats.get_time(),
                                                                                    ...
                                                                                    max_size, total_effort,
                                                                                    mean_agent_effort))

    firms_txt.close()


def save_firms_data(my_firms, decision):
    # Check whether the file has the same name. If TRUE, delete the file
    ...
    for firm in my_firms:
        firm_txt.write('%s, %s, %s, %s, %s, %s, %2f \n' % (statistics.my_stats.time, firm.unique_number,
                                                           firm.first_year, firm.last_year,
                                                           firm.get_age(statistics.my_stats.time), len(firm.members),
                                                           firm.output))
